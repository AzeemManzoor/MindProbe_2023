import React from 'react'
import { Col, Row } from "react-bootstrap";
import assessment from './assessment.css';

function PRE() {

const redirect = () => {

window.location.href ='/Assessment';


}



  return (
    <div>
        

        <Row>
  <div className='headT'>
    <h1>Free Personality Test</h1>
    <h4>By MindProbe</h4>
  </div>
</Row>

<Row className='row2-1'>
  <Col md={4} className='col'>
    <div className='card   card1' style={{ width: '18rem' }}>
      <img
        src='https://www.16personalities.com/static/images/test-header-2.svg'
        className='card-img-top1'
        alt='...'
      />
      <div className='card-body'>
        <p className='card-text'>
          Be yourself and answer honestly to find out your personality type.
        </p>
      </div>
    </div>
  </Col>

  <Col className='col' md={4}>
    <div className='card   card1' style={{ width: '18rem' }}>
      <img
        src='https://www.16personalities.com/static/images/academy/explorers/icons/theory.svg'
        className='card-img-top1'
        alt='...'
      />
      <div className='card-body'>
        <p className='card-text'>
          Learn how your personality type influences many areas of your life.
        </p>
      </div>
    </div>
  </Col>

  <Col className='col' md={4}>
    <div className='card   card1' style={{ width: '18rem' }}>
      <img
        src='https://www.16personalities.com/static/images/academy/analysts/exercise.svg'
        className='card-img-top1'
        alt='...'
      />
      <div className='card-body'>
        <p className='card-text'>
          Grow into the person you want to be with your optional Premium Guides.
        </p>
      </div>
    </div>
  </Col>
</Row>





<Row>

  <div className='tag1-1' >

    <h2 className='tag1' >Instructions</h2>
  </div>
  <h4 className="textt22 arm">Scroll to see more instructions</h4>

      <div className="Sidebar">
        <h5 className="textt1">Click on the Start Assessment button (below) to start the assessment.</h5>
        <h5 className="textt1">The questionnaire is divided into FIVE sections.</h5>
        <h5 className="textt1">You will be asked SIX questions in each section.</h5>
        <h5 className="textt1">Please provide brief and to-the-point answers. Please give answers honestly.</h5>
        <h5 className="textt1">You can also change questions by refreshing the page.</h5>
        <h5 className="textt1">Carefully provide answers, as after submitting, you will not be able to change them.</h5>
        <h5 className="textt1">After completing your answers, click on the submit button to proceed to the next section.</h5>
        <h5 className="textt1">In the last section, on clicking the submit button, your answers will be saved.</h5>
        <h5 className="textt1">Now, click the SUBMISSION button to submit all your answers and proceed to the next page.</h5>
        <h5 className="textt1">After submitting your answers, you will have an option to move to the REPORT page or try our Video Interview Feature.</h5>
        <h5 className="textt1">If you click on the Report button, you will be redirected to the REPORT page.</h5>
        <h5 className="textt1">If you click on the Video Interview button, you will be redirected to our Video Interview page.</h5>
        <h5 className="textt1">On reaching the Video Interview page, your interview will be started.</h5>
        <h5 className="textt1">Please make sure to sit in good lighting and focus your face on the front camera.</h5>
        <h5 className="textt1">You will be asked some important daily life questions.</h5>
        <h5 className="textt1">You will have to answer each question within the given time (10-15 seconds).</h5>
        <h5 className="textt1">After completing the interview, click on the Report button to go to the REPORT page.</h5>
        <h5 className="textt1">Here you can enjoy your report and learn more about yourself.</h5>
      </div>
      <h4 className="textt2">GOOD LUCK!</h4>
</Row>

<Row>
<h2 className='tag2' >FEATURES</h2>
</Row>


<Row className='pre-row' >

<Col md={6}>

<h3 className='coli1'>QUESTIONAIRE</h3>
<div className='coli' >
<ul>
<li><h5>Well Designed questions</h5>
</li>
<li><h5>Insightful Analysis</h5>
</li>
<li><h5> Personality traits</h5>
</li>
<li><h5>Convenient Interaction</h5>
</li>
<li><h5>Personality Report</h5>
</li>




</ul>

</div>
</Col>

<Col md={6} >

<h3 className='coli1'>QUESTIONAIRE plus INTERVIEW</h3>
<div className='coli' > 

<ul>
<li><h5>Well Designed questions</h5>
</li>
<li><h5>Insightful Analysis</h5>
</li>
<li><h5> Personality traits</h5>
</li>
<li><h5>Convenient Interaction</h5>
</li>
<li><h5>Personality Report</h5>
</li>
<li><h5>Authentic Engagement</h5>
</li>
<li><h5>Visual Insights</h5>
</li>
<li><h5>Comprehensive Understanding</h5>
</li>
<li><h5>Emotional Analysis</h5>
</li>
</ul>
</div>


</Col>

</Row>




<Row  className=' acc'>
{/* <a  href= > */}
  <button href='/Assessment' 
  onClick={redirect}
  className='ac-btn3 ac-bt'
>Start Assessment</button>
{/* </a> */}
</Row>

















    </div>
  )
}

export default PRE